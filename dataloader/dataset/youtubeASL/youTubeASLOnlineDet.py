...
class YouTubeASLOnlineDet(Dataset):
    def __init__(self, video_dir: str = None, num_frames_per_clip: int = 200, target_img_size = (224, 224), debug = False, return_frame = False):
        """
    ...
    def uniform_sample_frames_with_hand_filter(self, video_path: str, start_time: float, end_time: float, print_flag: bool = False) -> Optional[List[np.ndarray]]:
        ...
        sampled_num_frames = self.num_frames_per_clip if self.num_frames_per_clip < total_clip_frames else total_clip_frames
        frame_indices = np.linspace(start_frame, end_frame, sampled_num_frames, dtype=int)
        # frames = []
        selected_frames = []
        frame_index = []
        all_hand_keypoints = []
        ...
        for idx in frame_indices:
            ...
            if results.multi_hand_landmarks:
                ...
                #     frame_rgb = cv2.copyMakeBorder(frame_rgb, 0, 0, left_pad, right_pad, cv2.BORDER_CONSTANT, value=[0, 0, 0])
                # elif w > h:
                #     top_pad = (w-h)//2
                #     bottom_pad = w - h - top_pad
                #     frame_rgb = cv2.copyMakeBorder(frame_rgb, top_pad, bottom_pad, 0, 0, cv2.BORDER_CONSTANT, value=[0, 0, 0])
                
                # frame_bgr = cv2.cvtColor(frame_rgb, cv2.COLOR_RGB2BGR)

                padded_h, padded_w, _ = frame_rgb.shape
                # Initialize both hands as not detected
                right_hand_coords = [(-1, -1)] * 21
                left_hand_coords = [(-1, -1)] * 21
                for hand_landmarks, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
                    label = handedness.classification[0].label  # 'Left' or 'Right'
                    coords = []
                    for lm in hand_landmarks.landmark:
                        x = (int(lm.x * w) + left_pad)/ padded_w
                        y = (int(lm.y * h) + top_pad) / padded_h
                        coords.append((x, y))
                    if label == 'Right':
                ...
                all_hand_keypoints.append(right_hand_coords + left_hand_coords)
        ...
        return selected_frames, frame_index, all_hand_keypoints
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, str, dict]:
        ...
        # print(f"Video name: {video_name}, VTT name: {vtt_name}, Start time: {start_time}, End time: {end_time}, Text: {text}")
        video_path = os.path.join(self.video_dir, video_name)
        
        # Sample frames with hand filtering
        sampled_frames_rgb, sampled_frames_index, all_hand_keypoints = self.uniform_sample_frames_with_hand_filter(video_path, start_time, end_time)
        ...
        hand_keypoints_all = []
        body_keypoints_all, face_keypoints_all = [], []

        for frame_rgb in sampled_frames_rgb:
            _, body_kp, face_kp = self._detect_keypoints(frame_rgb, det_hand_kpts = False)  # 不检测手部关键点，因为已经在采样时检测过了
            # print('len(face_kp):', len(face_kp))
            body_keypoints_all.append(body_kp)
            face_keypoints_all.append(face_kp)

        hand_keypoints_all = all_hand_keypoints  # 这里已经是一个列表，包含了每帧的手部关键点
        ...
        if self.return_frame:
            sampled_frames_rgb = np.array(sampled_frames_rgb)

        pad_len = self.num_frames_per_clip - hand_keypoints_all.shape[0]  # 计算填充长度
        ...
        if self.return_frame:
            ...
            # Create a dummy tensor for frames
            frames_resized = torch.zeros((self.num_frames_per_clip, 1), dtype=torch.float32)

        # Create keypoints dictionary
        keypoints_dict = {
            'hand': hand_keypoints_all, # right hand + left hand
    ...
    def iou_filter(self, left_hand_bboxes, right_hand_bboxes, threshold=0.88):
        '''
    ...
    def fine_sample_frames_with_filter(self, video_path: str, start_time: float, end_time: float, drop_last: int, print_flag: bool = False) -> Optional[List[np.ndarray]]:
        ...
        # else:
        filtered_indices = self.iou_filter(left_hand_bboxes, right_hand_bboxes)
...
def main_sample_examples():
    dataset = YouTubeASLOnlineDet(num_frames_per_clip = 15)
