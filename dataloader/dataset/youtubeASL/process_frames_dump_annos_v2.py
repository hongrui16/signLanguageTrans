...
class youTubeASLDumpJson:
    ...
    def dump_anno(self, save_img_dir, save_json_dir, start_idx=0, end_idx=None):
        ...
        for i, idx in enumerate(to_be_processed_ids):
            ...
            import json
            json_filepath = os.path.join(save_json_dir, f'{video_name_prefix}_SID{clip_id}_anno.json')
            ...
            fps = 3
            output_path = os.path.join(save_img_dir,  f'{video_name_prefix}_SID{clip_id}_frames.mp4')
            if os.path.exists(output_path):
                try:
                    os.remove(output_path)
                except FileNotFoundError:
                    print(f"Warning: {output_path} was already deleted by another process.")
